...
pred_all <- predict(m1,
    newdata = hsbsub, re.form = NA,
    allow.new.levels = TRUE
)
prederr_all <- hsbsub$mAch - pred_all
...
prederr_all <- hsbsub$mAch - pred_all
mse_m3 <- mean(prederr_all^2, na.rm = TRUE)
pred_all <- predict(m4,
    newdata = hsbsub, re.form = NA,
    allow.new.levels = TRUE
)
prederr_all <- hsbsub$mAch - pred_all
mse_m4 <- mean(prederr_all^2, na.rm = TRUE)
tibble(Model = c("M1", "M2","M3","M4"), 
       `Out-of-sample MSE` = c(mse_m1, mse_m2,mse_m3,mse_m4))
```
...
Fill in the blank: Model 1 has the highest weight
...
Fill in the blanks: Aside from `ses_cm` and `ses_cmc`, minrty, sx and
sx\_cm are the most important for predicting `mAch`, while minrty\_cm is
the least important?
